...
#### Step 3 — Create IAM user group for EC2

1. Go to **AWS Console → IAM → IAM user groups → Create group**
2. Users: **Add user → IAM user access key you will use**
3. Attach these policies:
   - `AmazonSSMManagedInstanceCore` — enables tunnel access without SSH
   - `AmazonEC2FullAccess` — allows instance to terminate itself
   - `AmazonSSMFullAccess` — allows full ssm control for reading\writing runner token and debuging instance via ssm session
4. Attach these inline policies:
   - `PassRole-Allow-SSM` — enables tunnel access without SSH
   - `SSM-ALLOW-ALL` — allows instance to terminate itself
5. Name the role: `ec2-github-runner-group`
...
#### Step 5 — Store GitHub Runner Token in SSM
...
#### Step 6 — Check deploy_runner.sh

**Edit the variables at the top** to match your AWS account:
``` bash
AMI_ID="ami-0122da1d0b4a30ec4" 
INSTANCE_TYPE="t3.micro"
IAM_PROFILE="arn:aws:iam::382720393134:instance-profile/ec2-github-runner"
SUBNET_ID="subnet-XXXXXXXXXX"
SG_ID="sg-XXXXXXXXX"
```

and edit **repo link** in this command:
...
Make sure `package.json` has test command configured:
...
# 4. Manually trigger the test workflow
...
| `Must not run with sudo` | Running runner as root | Use `RUNNER_ALLOW_RUNASROOT=1` |
